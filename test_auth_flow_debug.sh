...
# Debug version of the test script for the MediCare FHIR API authentication flow
# This script tests the complete flow from admin login to patient/practitioner registration and resource access
# with additional debugging steps
...
function make_request() {
    ...
    if [ ! -z "$data" ]; then
        ...
        result=$(curl -s -X $method "$API_URL$endpoint" -H "Content-Type: application/json" ${token:+-H "Authorization: Bearer $token"} -d "$data")
        echo -e "${YELLOW}Raw response:${NC}"
        echo "$result"
        return
    else
        echo -e "${YELLOW}Request: $method $API_URL$endpoint${NC}"
        result=$(curl -s -X $method "$API_URL$endpoint" -H "Content-Type: application/json" ${token:+-H "Authorization: Bearer $token"})
        echo -e "${YELLOW}Raw response:${NC}"
        echo "$result"
        return
    fi
...
echo -e "${YELLOW}Trying to parse response with jq:${NC}"
echo $ADMIN_LOGIN_RESPONSE | jq . || echo -e "${RED}Failed to parse JSON${NC}"

# Extract admin token
ADMIN_TOKEN=$(echo $ADMIN_LOGIN_RESPONSE | jq -r '.data.accessToken' || echo "")
echo -e "${YELLOW}Extracted token: $ADMIN_TOKEN${NC}"
...
# Step 1.5: Verify access codes collection is empty or check existing codes
echo -e "${BLUE}Step 1.5: Checking existing access codes${NC}"
ACCESS_CODES_RESPONSE=$(make_request "GET" "/access-codes" "" "$ADMIN_TOKEN")
echo $ACCESS_CODES_RESPONSE | jq .
echo -e "${GREEN}Access codes check complete${NC}\n"
...
PATIENT_RESPONSE=$(make_request "POST" "/fhir/Patient/with-access-code?email=$PATIENT_EMAIL" "$PATIENT_DATA" "$ADMIN_TOKEN")
...
# Step 2.5: Verify the access code was created correctly
echo -e "${BLUE}Step 2.5: Verifying patient access code${NC}"
VERIFY_CODE_RESPONSE=$(make_request "POST" "/access-codes/verify" "{\"code\":\"$PATIENT_ACCESS_CODE\"}" "$ADMIN_TOKEN")
echo $VERIFY_CODE_RESPONSE | jq .
echo -e "${GREEN}Access code verification complete${NC}\n"
...
PRACTITIONER_RESPONSE=$(make_request "POST" "/fhir/Practitioner/with-access-code?email=$PRACTITIONER_EMAIL" "$PRACTITIONER_DATA" "$ADMIN_TOKEN")
...
# Step 3.5: Verify the access code was created correctly
echo -e "${BLUE}Step 3.5: Verifying practitioner access code${NC}"
VERIFY_CODE_RESPONSE=$(make_request "POST" "/access-codes/verify" "{\"code\":\"$PRACTITIONER_ACCESS_CODE\"}" "$ADMIN_TOKEN")
echo $VERIFY_CODE_RESPONSE | jq .
echo -e "${GREEN}Access code verification complete${NC}\n"
...
PATIENT_REGISTER_RESPONSE=$(make_request "POST" "/auth/register" "$PATIENT_REGISTER_DATA")
...
PRACTITIONER_REGISTER_RESPONSE=$(make_request "POST" "/auth/register" "$PRACTITIONER_REGISTER_DATA")
...
PATIENT_PROFILE_RESPONSE=$(make_request "GET" "/auth/me" "" "$PATIENT_TOKEN")
...
PATIENT_FHIR_RESPONSE=$(make_request "GET" "/fhir/Patient/$PATIENT_RESOURCE_ID" "" "$PATIENT_TOKEN")
...
PRACTITIONER_PROFILE_RESPONSE=$(make_request "GET" "/auth/me" "" "$PRACTITIONER_TOKEN")
...
PRACTITIONER_FHIR_RESPONSE=$(make_request "GET" "/fhir/Practitioner/$PRACTITIONER_RESOURCE_ID" "" "$PRACTITIONER_TOKEN")
...
PRACTITIONER_ACCESS_PATIENT_RESPONSE=$(make_request "GET" "/fhir/Patient/$PATIENT_RESOURCE_ID" "" "$PRACTITIONER_TOKEN")
...
PATIENT_ACCESS_PRACTITIONER_RESPONSE=$(make_request "GET" "/fhir/Practitioner/$PRACTITIONER_RESOURCE_ID" "" "$PATIENT_TOKEN")
